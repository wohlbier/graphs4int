...
class HighOrderAggregator(Layer):
    ...
    def __init__(self, dim_in, dim_out, dropout=0., act='relu', \
            order=1, aggr='mean', is_train=tf.constant(1), bias='norm', **kwargs):
        ...
        self.order = order
        self.aggr = aggr
        self.is_train = tf.equal(is_train, tf.constant(1))
        if dim_out > 0:
            with tf.compat.v1.variable_scope(self.name + '_vars'):
...
class AttentionAggregator(Layer):
    ...
    def __init__(self, dim_in, dim_out,
            dropout=0., act='relu', order=1, aggr='mean', is_train=tf.constant(1), bias='norm', **kwargs):
