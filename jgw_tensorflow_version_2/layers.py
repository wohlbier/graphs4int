...
from graphsaint.jgw_tensorflow_version_2.inits import glorot,zeros,trained,ones,xavier,uniform
...
class JumpingKnowledge(Layer):
    ...
    def _call(self, inputs):
        ...
        vw = tf.compat.v1.matmul(feats_aggr,self.vars['weights'])
        vw += self.vars['bias']
        vw = self.act(vw)
        if self.bias == 'norm':
...
class HighOrderAggregator(Layer):
    ...
    def _F_nonlinear(self,vecs,order):
        vw = tf.compat.v1.matmul(vecs,self.vars['order{}_weights'.format(order)])
    ...
    def _call(self, inputs):
        ...
        for o in range(self.order):
            for a in range(o+1):
                ans1 = tf.compat.v1.sparse.sparse_dense_matmul(adj_norm,vecs_hop[o+1])
                ans_partition = [tf.compat.v1.sparse.sparse_dense_matmul(adj,vecs_hop[o+1]) for adj in adj_partition_list]
...
class AttentionAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        adj_norm = tf.cond(pred=self.is_train,true_fn=lambda: adj_norm,false_fn=lambda: tf.compat.v1.sparse.concat(axis=0,sp_inputs=adj_partition_list))
        vecs_do1 = tf.nn.dropout(vecs, 1 - (1-self.dropout))
        vecs_do2 = tf.nn.dropout(vecs, 1 - (1-self.dropout))
        vw_self = tf.compat.v1.matmul(vecs_do2,self.vars['order0_weights'])
        ...
        for i in range(self.mulhead):
            vw_neigh.append(tf.compat.v1.matmul(vecs_do1,self.vars['order1_weights_h{}'.format(i)]))
        ...
        for i in range(self.mulhead):
            ...
            ret_neigh_i = self.act(tf.compat.v1.sparse.sparse_dense_matmul(adj_weighted,vw_neigh[i])) \
